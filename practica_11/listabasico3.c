...
int main(void)
{
	...
	// *** -----------------------------------
	int buscarenodo;
	printf("\nNUMERO PARA BUSCAR EN LOS NODOS Y BORRARLO:");
	scanf("%d", &buscarenodo);
	if (lst == NULL)
	{
		printf("\nLISTA VACIA\n");
	...
	printf("\nINDICE DEL NODO AL QUE QUIERES ACCEDER: ");
	scanf("%d", &ind);
	if (lst == NULL)
	{
		printf("ERROR");
	...
	if (i == ind)
	{
		printf("\nEl numero en el indice %d es %d", ind, aux->info);
	}
	else
	...
	// *** -----------------------------------
	tipoNodo *ant;
	ant = (tipoNodo *)malloc(sizeof(tipoNodo));
	int cont = 1;
	printf("\nELIMINAR NODO SITUADO EN LA POSICION: ");
	scanf("%d", &ind);
	if (lst == NULL)
	{
		printf("LISTA VACIA\n");
	...
	// ***     Por ejemplo, si la lista tiene 4 nodos (los numeramos de 0 a 3), y se
	// ***     ha indicado como posicion 6, el nodo se inserta ultimo
	// *** -----------------------------------

	nuevo = NULL;

	printf("\nINSERTAR NODO SITUADO EN LA POSICION: ");
	scanf("%d", &ind);
	printf("QUE DATO DESEA GUARDAR EN ESE NODO: ");
	scanf("%d", &nuevo->info);
